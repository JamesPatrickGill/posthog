...
3. **start_agent_server** — Runs `npx agent-server` inside the sandbox and polls
   ...
   `SANDBOX_RUST_CLAUDE_DRIVER` and `SANDBOX_RUST_CODEX_DRIVER` additionally export `POSTHOG_CLAUDE_ADAPTER_CMD=/usr/local/bin/claude-acp-driver` / `POSTHOG_CODEX_ADAPTER_CMD=/usr/local/bin/codex-acp-driver`, which the Rust agent-server applies to runs of the matching adapter only, replacing the Node ACP sidecar with the native driver.
