...
import { KAFKA_APP_METRICS_2, KAFKA_HOG_INVOCATION_RESULTS, KAFKA_LOG_ENTRIES } from '~/common/config/kafka-topics'
...
// Janitor poison-pill recovery — the incident fix. A janitor give-up must never
// silently drop a run: during a fleet-wide outage stalls are paused (jobs stay
// recoverable), and an isolated bad job is recorded as a failed, replayable
// invocation result on `hog_invocation_results` BEFORE its cyclotron row is
// deleted. postgres-v2 only (the Cyclotron Node backend the janitor manages).
describe('Workflows E2E (janitor poison-pill recovery, postgres-v2)', () => {
    ...
    // Insert a poison pill: a hogflow job stuck 'running' with a long-stale
    // heartbeat and a touch count over the budget. The serialized state mirrors
    // a wait_until_condition that had already advanced past earlier actions.
    ...
    it('pauses giving up during a fleet-wide stall, leaving every job recoverable', async () => {
        const ids: string[] = []
        for (let i = 0; i < 6; i++) {
            ids.push(await insertPoisonedHogflowJob())
        }

        janitor = createJanitor()
        const result = await janitor.runOnce()

        // Stalls dominate → the give-up is paused; nothing is recorded or dropped.
        ...
        // All six survive, reset to available so a recovered worker can drain them.
        const rows = await cyclotronPool.query(`SELECT status FROM cyclotron_jobs WHERE id = ANY($1::uuid[])`, [ids])
        expect(rows.rowCount).toBe(6)
        expect(rows.rows.every((r: any) => r.status === 'available')).toBe(true)
    })
})
