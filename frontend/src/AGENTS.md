...
Run `pnpm --filter=@posthog/frontend fix` (oxlint `--fix` + oxfmt) before finishing; `format` runs oxfmt only. Config: root `.oxlintrc.json`. CSS, spelling, and copy-casing rules live in the root `AGENTS.md` (Code Style).
