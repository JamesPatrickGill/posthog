...
class TestRefreshSandboxGithubForUser:
    ...
    def _clear_identity_cache(self):
        from products.tasks.backend.temporal.process_task.utils import clear_sandbox_identities

        clear_sandbox_identities("run-gh-1")
        yield
        clear_sandbox_identities("run-gh-1")
    ...
    def test_swaps_token_and_git_author_and_marks_identity(self):
        ...
        assert get_last_sandbox_identity("run-gh-1", "github_user") == 99
    ...
    def test_switching_back_to_creator_reapplies_their_identity(self):
        ...
        mark_sandbox_identity("run-gh-1", "github_user", 99)
        sandbox = MagicMock()
        sandbox.is_running.return_value = True
        ...
        assert get_last_sandbox_identity("run-gh-1", "github_user") == 1
