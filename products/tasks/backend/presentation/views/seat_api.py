...
class SeatViewSet(viewsets.ViewSet):
    ...
    @action(detail=True, methods=["post"], url_path="switch-to-usage-based")
    def switch_to_usage_based(self, request: Request, pk: str | None = None) -> Response:
        """POST /api/seats/me/switch-to-usage-based/ -> POST /api/v2/seats/{distinct_id}/switch-to-usage-based/

        Not gated by ``SEAT_PURCHASES_DISABLED_FLAG`` — this is the destination of the
        seat-purchase lockdown, not a purchase itself. Billing's response (including a
        400 with a ``redirect_url`` when the org has no subscription) passes through
        unmodified so the client can act on it.
        """
        if pk != "me":
            self._require_admin(request)
        ...
        resp = self._billing_request(
            "POST",
            f"/api/v2/seats/{distinct_id}/switch-to-usage-based/",
            headers,
            json_body=request.data,
        )
        return self._forward_response(resp)
