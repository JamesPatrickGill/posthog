...
4. **Wait for ClickHouse** (`wait_for_signal_in_clickhouse_activity`): a two-tier poll tuned by `WaitForClickHouseMode`. Unless the mode is `ch_only`, every attempt first checks the embedding worker's recently-seen store (a cheap key-value lookup over `posthog.api.embedding_worker.async_get_recently_seen_documents`); once the store reports every signal's `emitted_at` at or after the signal's own timestamp, the wait either ends outright (`optimistic` — grouping and report deletion) or runs the ClickHouse confirmation query (`ch_confirmed` — bulk reprocessing). For the first 5 minutes the wait is otherwise store-exclusive; after that ClickHouse also polls on every 3rd attempt (a third of the store's rate) as a fallback, since the store is best-effort. `ch_only` (interactive reingestion) skips the store and polls ClickHouse every attempt, because a soft-delete re-emission reuses the original timestamp and is therefore indistinguishable from the original emission in the store. The ClickHouse query counts emitted `document_id`s within a widened timestamp range (`min(timestamp)-2m` to `max(timestamp)+2m`) and `inserted_at >= now() - 30 minutes`, which avoids matching stale earlier emissions of the same IDs while tolerating precision loss and queueing delay. This query intentionally does **not** filter on `deleted`.
