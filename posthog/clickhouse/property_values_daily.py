...
PROPERTY_VALUES_DAILY_TABLE_BASE_SQL = """
CREATE TABLE IF NOT EXISTS {table_name}
(
    ...
    `property_value` String,
    `day` Date DEFAULT toDate(now()){extra_fields}
) ENGINE = {engine}
"""


def PROPERTY_VALUES_DAILY_TABLE_SQL() -> str:
    return (
        PROPERTY_VALUES_DAILY_TABLE_BASE_SQL
        + """
PARTITION BY day
...
"""
        ...
        extra_fields=""",
    INDEX idx_property_value lower(property_value) TYPE text(tokenizer = splitByNonAlpha)""",
    )


def KAFKA_PROPERTY_VALUES_DAILY_TABLE_SQL_FN() -> str:
    return KAFKA_PROPERTY_VALUES_DAILY_TABLE_SQL.format(
        table_name=KAFKA_TABLE_NAME,
        engine=kafka_engine(
            topic=KAFKA_CLICKHOUSE_PROPERTY_VALUES,
            group=CONSUMER_GROUP_PROPERTY_VALUES_DAILY,
            named_collection=settings.CLICKHOUSE_KAFKA_WARPSTREAM_INGESTION_NAMED_COLLECTION,
        ),
    )


def DROP_KAFKA_PROPERTY_VALUES_DAILY_TABLE_SQL() -> str:
    return f"DROP TABLE IF EXISTS {KAFKA_TABLE_NAME}"


def DROP_PROPERTY_VALUES_DAILY_MV_SQL() -> str:
    return f"DROP TABLE IF EXISTS {MV_NAME}"


def PROPERTY_VALUES_DAILY_MV_SQL() -> str:
    return """
...
def DISTRIBUTED_PROPERTY_VALUES_DAILY_TABLE_SQL() -> str:
    return PROPERTY_VALUES_DAILY_TABLE_BASE_SQL.format(
